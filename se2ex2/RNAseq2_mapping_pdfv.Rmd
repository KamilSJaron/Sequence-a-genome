...
2. In case of successful trimming, move trimmed reads to apropriate folder
...
Now we have a read counts (outputs saved in folders) and the pseudoalignment file. The `.sam` file produced by kallisto is mapping reads relatively to transctipts, what we want is to visualise reads on whole genome. Since direct usage of `bsub` with command is not that streightforward with pipes, we will use a script `submit_conversion.sh` to submit a job.

1. Copy scipts `kallisto_sam_convertor.py` and `submit_conversion.sh` to your directory from 
...
Script `kallisto_sam_convertor.py` redefines a `.sam` file for `IGV` and prints it on standard output. The redefined file we want to compress, sort and index, therefore we can pipe the output of the script to `samtools`. 
...
IGV needs to build an index on the `.bam` file. Building an index is not too much computationally inensive, you can run the command directly on `prd` computer or use a priority queue.
...
4. Check if you see following screen with loaded genome and annonation
