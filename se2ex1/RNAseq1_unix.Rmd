...
0. Log to Vital-it
1. Go to your working folder (`/scratch/cluster/monthly/mls_2015/<username>`) and create an output directory for results of quality control (use meaningful name, for instance `<condition><bio_rep>_qc` could be a good idea)
...
3. Check `bjobs` to see status of your job and email if you had a problem (when job is finished, you will get an email).
...
```bash
bsub -n 8 -M 8388608 -J <job_name> trimmomatic SE -threads 8 LM2.fastq.gz LM2t.fastq.gz ILLUMINACLIP:<adapters.fasta>:3:25:6 LEADING:9 TRAILING:9 SLIDINGWINDOW:4:15 MINLEN:60
```
...
Credits: made by Andrea and Kamil, Contact: kamil.jaron@isb-sib.ch
