...
out$path_to_reads <- paste('/scratch/cluster/monthly/kjaron/sag/RNAseq',out$con, paste(out$con,out$bio_repl,'.fastq.gz', sep = ''), sep = '/')
...
<sub>
The `fastqc` is program designed for genomic reads in first place, therefore some red flags are not crucial for us. For instance funky QC content or kmer bias are actually expectable properties of RNA seqeunces. What is important is to have high quality data, no adapter content and not too short reads.
<sub\>
...
<sub>
sometimes, when the fragment is really small, there can be more consetutive adapters sequenced. Those adapters are not cutted in demultiplexing in the sequencing center. It is up on us, to get rid of them. The most of possible adapters of Illumina RNAseq are in the file `/scratch/cluster/monthly/kjaron/sag/RNAseq/RNAseq_adapters.fa`. `trimmomatic` cat try to identify those sequences in the reads and cut them off if the module `ILLUMINACLIP` is used. You have to specify the file with adapters. Other modules of trimmomatic, `LEADING`, `TRAILING` are cutting givenn number bases from the start and end of read if the quality is bellow some internal treshold. Module `SLIDINGWINDOW` performs a sliding window trimming, cutting once the average quality within the window falls below a threshold. Module `MINLEN` simply discards all reads which are after all that trimming shorter than given number of bases.
<sub\>
...
You will need `R` and `IGV` on your computer and programs `fastqc`, `Kallisto` and `samtools` prepared to use on Vital-it or on your computer (OS X, Linux). Make your mind if you want to run programs locally or on the cluster and follow one of the guides.
...
Credits: made by Andrea and Kamil with kind help of Amina and Olivier. Contact: kamil.jaron@isb-sib.ch
