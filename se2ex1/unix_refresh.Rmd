...
2. copy directory `/scratch/cluster/monthly/kjaron/unix_ex` to your working directory using command `cp` (copy)
...
4. Standard output is usually printed to terminal (list of files in case of `ls`, content of file in case of `cat`, etc.). Using symbol `|` you can redirect ("pipe") the standard output as an input to another command. We can use a few commands to find out how many nucleotides are in the genome of *Pseudomonas protegens* have sequenced last semester. File `Pseud_protegens_S5_final.fasta` is a simple fasta file, so number of nucleotides is equal to number of all no newline characters in the file without the first line (header).

```bash
head -n 1 Pseud_protegens_S5_final.fasta | wc -c # returns the number of characters in the first line
wc Pseud_protegens_S5_final.fasta -lc # returns the number of newline character and the number of all characters in the file
```
...
<sub>
The sequences in `.fasta` files are usually stored in 60 nucleotides per line, but bioinformatians are messy and the format of the `.fasta` slightly differs sometimes. In our case there are no newline characters in the sequence (i.e. the whole gneome is in the second line of the `.fasta` file). Also, if our assembly would have several contigs, there should be more header lines in the files. Do not get confused, that amost all softwares are automatically wrapping too long line to make a document more readible. Using `less Pseud_protegens_S5_final.fasta` gives you a feeling of a usuall `.fasta` even we already know, that is not the case.
</sub>

5. You can simplify your problem using command `tail` which can also print all but a given number of lines from begging. Following command prints number of characters. Is it same as before? 
