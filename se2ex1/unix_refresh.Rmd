...
* blocks of code (bash or R) will be in boxes, note also the usual syntax of Unix commands `command -parameters location`. Comments in code are after the `#` symbol.
...
cp -r /scratch/cluster/monthly/kjaron/sag/unix_ex . #directories need to be copied using -r (stands for recursive)
```
...
4. Standard output is usually printed to terminal (list of files in case of `ls`, content of file in case of `cat`, etc.). Using symbol `|` you can redirect ("pipe") the standard output as an input to another command. We can use a few commands to find out how many nucleotides are in the genome of *Pseudomonas protegens* we have sequenced last semester. File `hox_bflo.fa` is a simple fasta file, so the number of nucleotides is equal to the number of all characters other than newline in the file without the first line (header).

<sub>
An alternative to pipe is the symbol `>`, which serves for redirection of standard output stream to a file. Be careful, if the target file exists, it will be overwritten. To add lines to the end of the line, you can use `>>` (= concatenate).
</sub>

```bash
head -n 1 hox_bflo.fa | wc -c # returns the number of characters in the first line
wc hox_bflo.fa -lc # returns the number of newline characters and the number of all characters in the file
```
...
<sub>
The sequences in `.fasta` files are usually stored in 60 nucleotides per line, but bioinformatians are messy and the format of the `.fasta` slightly differs sometimes. In the case of our genome there are no newline characters in the sequence (i.e. the whole gneome is in the second line of the `.fasta` file). Do not get confused, amost all softwares are automatically wrapping lines which are too long to make a document more readable. Using `less hox_bflo.fa` gives you a feeling of a usual `.fasta` even if we already know that is not the case with ours. Also, if our assembly would have several contigs, there should be more header lines in the files. 
</sub>
...
1. Extract all header lines of `LM2_L1_R1.fastq` file to separated file. If you do the same on `hox_bflo.fa`, how many headers do you extract?
...
- Press tabulator to call whisperer (autocompletion) every time you are writing a path. It is also handy with long filenames as we can find in genomics (Note that you will have to type all the first letters of the file which are in common with other filenames of same the directory before autocompletion can be achieved by pressing tabulator). It is a good way how to prevent typos.
