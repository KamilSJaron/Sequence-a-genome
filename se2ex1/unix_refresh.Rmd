...
java -version
```
if is it fine, download fastqc from [Babraham bioinformatics webpage](http://www.bioinformatics.babraham.ac.uk/projects/fastqc/). Unzip the file. Go to the folder, change permissions to make `fastqc` executable and link it to your `/usr/bin`.
```bash
chmod u+x fastqc
...
- Bash script `~/.bashrc` is executed every time you open new terminal.
