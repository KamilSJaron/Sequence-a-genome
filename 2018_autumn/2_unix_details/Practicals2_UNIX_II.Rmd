...
We already know the basics of unix. You know how to write commands that perform basic operations on files. We have also learned that there are some special symbols that mean something (like `|`, `>`, etc.). Now we would like to explore more of these characters.

`bash` is actually preprocessing your input commands. If you use some of the special symbols, `bash` will replace them according their meaning. Since these replacement operations might be bit confusing at the beginning we will need to just print the command without actually executing it. This is performed using a command `echo`. If you write something like the line bellow

```{bash}
echo this_does_not_expand *.Rmd $HOME
```

`bash` has performed all possible expansions on the `this_does_not_expand *.Rmd $HOME` and then `echo` printed it. To understand what actually happened just continue in the practical.

## Wild characters

A wild character, asterisk (little star, `*`), is a symbol interpreted by `bash` as "all possible content".
You can combine it with strings to show only subset of files containing the string (example `*.fasta` means, all files with suffix `.fasta`, `reads_*` means all files starting with `reads_`.). This is a substitution that happens before the execution of a command.
...
**Note from Natasha: should there be a part explaining strings, ints, floats, datatypes in general (at least in the presentation)?"

## Variables

`bash` also allows for the use of variables, similar to `R` or `python`. You can define them by `<varible_name>=<value>`, where `<varible_name>` is usually by convention upper-case and `<value>` can be either a number or a string (text).
...
<sub> Note : We will play bit more with files located at `/scratch/beegfs/monthly/mls_2017/UNIX_testfiles/`, be sure that you have a copy in your working directory.</sub>
...
Your home is back-upped and you can not save more than 2GB there. All the data for computations should be stored on a disc called "scratch". `/scratch/beegfs/monthly` is a working place, all files and folders older than one month are automatically deleted. From now on, keep all your big files here (or on your computer). The quota for disc `beegfs` is 5TB per user. The quota for `home` is 2GB. Therefore, all jobs run from working directories are located at `/scratch/beegfs/monthly/mls_2015/<username>`.
