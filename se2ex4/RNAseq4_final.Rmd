...
(PLACE WHERE TESTING SHOULD IN FEW SENTENCES)
...
tr_counts <- readDGE(files, 
                     columns = c(1,4), 
                     group = rep(c('LM','SA','WL','WR'), each = 4), 
                     labels = conditions)$counts
...
## mabye stronger filtering? Like this is filter overall one value...
sum(!keep)

d <- DGEList(counts = tr_counts_clean, 
             group = rep(c('LM','SA','WL','WR'), each = 4))
...
de <- exactTest(d, pair = c("WL","WR"))

## for every gene you have performed a test, check histogram of p-values of all tests. WHat distribution you expect you would observe a differential expression in same biological condition? Do you remember what was FPR?
...
tT <- topTags(de, n = nrow(d), p.value = 0.05)$table

write.csv(tT, file = "diff_exp_WL_WR.csv")
```
...
\footnotesize
This is just an example, I lets say, that I have anotated genes "S5_genome_201" to "S5_genome_250".
\normalsize

2. Take a subset of defferentatinally expressed genes from those you have annotated. Are some of them differentially expressed?
