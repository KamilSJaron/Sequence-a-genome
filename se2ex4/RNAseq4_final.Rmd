...
We expected, that you will use this document as manual of tools for your analysis, so we have kept here the R part from the last week. We recommend you to add the rest of donwstream analysis to the same R script, you have created last week. Do not hesitate write comments in your code.
...
## 8. DIFFERENTIAL EXPRESSION ANALYSIS OF GENES (gene-level)
...
2. Create MA plot of the differentiall exprssed genes
...
## 8b. Differential expression of transcripts (transcript-level)
...
For every gene you have performed a test, check histogram of p-values of all tests. What distribution you expect you would observe a differential expression in same biological condition? Do you remember what was FDR?


```{r, echo = F}
hist(de.tr$table$PValue, breaks = 50, xlab = 'p-value (without correction)')
```

```{r}
## check what topTags function does by ?topTags
...
A gene clustering heatmap is a two-dimensional, rectangular, colored grid, representing each data point (rectangle) with a color that quantitatively and qualitatively reflects the original experimental observations. 
...
## to know what does %in%, check the ?match function :)
...
RPKM <- rpkm(rawcounts, geneLengths$length)
```

2. Make a subset of RPKMs of differentially expressed genes. Use log transformation.

```{r}
# taking top 100 deg to do the heatmap
log_rpkm.topgenes <- as.matrix(log2(RPKM[rownames(deg.list[1:100,]),]+1))
```

3. Create a heatmap of log RMKMs of those genes.
...
\footnotesize
Example: I have anotated genes "S5_genome_201" to "S5_genome_250".
\normalsize

2. Take a subset of differentially expressed genes/transcripts from those you have annotated. Are some of them differentially expressed?

```{r}

# deg.list comes from the beginning of this file
...
```{r}
log_rpkm.mygenes <- as.matrix(log2(RPKM[df_annotated,]+1))
