...
Since we assume, that you will use this document as manual of tools for your analysis, we have kept here the R part from the last week.
...
## this time we need to work with data frame instead of matrix
...
RPKM <- rpkm(rawcounts, geneLengths.merged$length)
```
...
log_rpkm.topgenes <- as.matrix(log2(RPKM[row.names(deg.list[1:100,]),]+1))
```
...
```{r}
annotated <- paste0('S5_genome',200:250)
```

2. are some of them differentially expressed?

```{r}
sum(annotated %in% rownames(deg.list))
```

3. \textit{a} If you have less than 100 genes: Make a heatmap.
3. \textit{b} If you have more than 100 genes: Do a MDS plot.
