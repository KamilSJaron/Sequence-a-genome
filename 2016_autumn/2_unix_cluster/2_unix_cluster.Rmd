...
since terminal is primarly trying to execute commands, variables has to be called using a special symbol `$`. A `bash` command that simply prints its arguments is called `echo` (similar to `print` or `message` in `R`).
...
In fact, we would like to see hidden files almost all the time, but specifying an argument `-a` of `ls` every single time is just... annoying. The way out is a creation of aliases (shortcuts) to commands you frequently use. You can


Very popular alias is `alias ll='ls -lah'`. If you add this line to your `~/.bashrc` you can use from now on command `ll` instead of `ls -lah` (list all, long format, in human readable units).

## Prepare a workspace

Before anything, keep in mind that you can save a lot of energy and mistakes by using `<tab>` for auto completion of all existing pathes you type and by using your command history (up and down arrows and `Ctrl+r`)
