...
Here are more materials about [UNIX](http://edu.isb-sib.ch/course/view.php?id=82).
...
Command `grep` searches the file and prints all lines containing a pattern specified by a parameter: `grep <pattern> <file.txt>`. I heavily recommend to write pattern in quotes, because if it contains any characters interpreted in command line (like spaces, symbols ">", "|", ".", "\\"...)
...
One the most powerful and simple tools for reading text files in terminal is `less`. It is loading to memory only parts of opened file that are read, therefore you can use this tool to check files of any sizes (even files exceeding the operational memory of your computer). To browse file you can use arrows, page-up/down or scroll of a mouse.
...
Editing files can be done using editors like `nano <textfile>` or `vi <textfile>`. Open `browsing.txt` in `vi`, press `a` to enter the editing mode. 

1. Correct one of the typos in text (for instance, second line: `enther` -> `enter`). Press `<Esc>` to escape from the writing mode. Press `:` to enter to command mode, write `wq` to save changes (write) and exit back to terminal (quit). Verify, that you managed edit the file.
...
2. Once you finish with Challenges you would like to achieve, delete whole directory `UNIX_testfiles`
