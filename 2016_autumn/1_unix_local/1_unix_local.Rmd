...
Here are more materials about [UNIX](edu.isb-sib.ch/course/view.php?name=UNIX_I).
...
Command `grep` searches the file and prints all lines containing a pattern specified by a parameter: `grep <pattern> <file.txt>`. We heavily recommend to write such patterns in quotes "", because characters can have a meaning in the command line with will interfer with what you want to do (for example spaces, or the symbols ">", "|", ".", "\\", etc.).
