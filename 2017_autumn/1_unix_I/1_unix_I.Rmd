...
1. Create a working directory `/home/<username>/UNIX_testfiles` and enter it.
2. Download a directory from address `https://raw.githubusercontent.com/KamilSJaron/Sequence-a-genome/master/2017_autumn/data/README.md`

```bash
wget https://raw.githubusercontent.com/KamilSJaron/Sequence-a-genome/master/2017_autumn/data/README.md
```

3. Copy  to your working directory using command `cp` (copy)
...
4. Go inside and check the content (you should know how, if you don't, scroll up).
5. Copy file `browsing.txt` in the directory `nonemptydir`. Do you need the parameter `-r`? If you are not sure, check manual page (`man cd`, for all inbuilt commands there is a manual page you can check).
6. Go to `nonemptydir`, rename `browsing.txt` to `<your_new_name>.txt` and move it back to `UNIX_testfiles`.
